 ...
 socket.on('connection', function (socketId) {
 	socket.emit('teamBallPlease', socketId)

 });

 socket.on('loginRegister', function(data){
 	$('.mainContent').html(data);
 });

 $('#TBlogin').on('click', function(){
	console.log('CLCIK LOgIN')
})

var dataObject={};
function TBlogin(username, password){
	var dataObject = {
		username : arguments[0],
		password : arguments[1]
	}
	socket.emit(dataObject)
	return dataObject
}
