...
app.post("/uploads", function(req, res) {
  ...
  if (app.get("username") === null) {
    logger.log("were going to get an error");
  }
  ...
  form.uploadDir = path.join(
    app.get("clientDir") + "users/" + app.get("username")
  );
      ...
      collection.update(
        ...
        function(err, r) {
          if (err) {
            ...
            "Ok no error updating user locations Array " + r;
          }
        }
      );
      collection.update(
...
io.on("connection", function(socket) {
  ...
  logger.log(
    "socketArray = " + socketArray.length + " added id = " + socket.id
  );
    ...
    if (username === "" || password === "") {
      ...
      MongoClient.connect(url, function(err, db) {
        if (!err) {
          ...
          var collection = db.collection("allusers");
          logger.log(
            "We are connected to " + db.databaseName + ", poolSize = "
          );
                ...
                if (r[0].password === password) {
                  fs.stat(app.get("clientDir") + "users/" + username, function(
                    ...
                    if (err) {
                      ...
                      logger.log("THIS USER IS READY TO UPLOAD FILES!!!");
                    }
                  });
                        ...
                        collection.update(
                          ...
                          function(err, r) {
                            if (err) {
                              ...
                              logger.log(
                                "update and set the user login info : returned " +
                                  r
                              );
                            }
                          }
  ...
  function runPrimitiveInChildProcess(fileName, username) {
    ...
    if (primitiveStateChecker === true) {
      ...
      logger.log(
        "Primitive photo action was emited for the file " +
          __dirname +
          "/client/users/" +
          username +
          "/" +
        ...
        db.collection("allusers").update(
          ...
          function(e, r) {
            if (e) {
              ...
              logger.log("user loged out and DM was updated");
            }
          }
        );
      }
